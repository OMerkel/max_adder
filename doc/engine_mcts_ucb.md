# UCT / MCTS Engine - Max Adder

This document describes the AI engine used by the Max Adder project:
...
- Exploration constant is effectively fixed at $\sqrt{2}$ (hardcoded form
  above).
- This implementation does not currently expose exploration constant as a
  runtime parameter.
...
The `Uct.getActionInfo(...)` parameters control breadth, depth, and
responsiveness.
...
With a larger value, each rollout can evaluate richer scoring and availability
outcomes, but each rollout also becomes more expensive. With a smaller value,
the AI can run more rollouts in the same time, but each one is shorter and
noisier.
...
When these depth limits are higher, each rollout can see farther, but each
rollout also costs more. That means fewer total iterations can fit inside the
same time budget.
...
In other words, you are balancing two goals: stronger tactical reading and
faster response time. Increasing all limits at once is not automatically better,
because
...
Horizontal-player difficulty, vertical-player difficulty, and device profile
are selected in the UI and consumed by worker logic in `js/controller.js`.
...
The worker receives the resolved profile and selects budgets from the active
profile table. For each AI turn, it chooses the role-specific difficulty by
active player:

- player controlling the horizontal marker -> `difficultyHorizontal`
- player controlling the vertical marker -> `difficultyVertical`
...
- immediate score-leading or terminal move preference
- selection when the active marker has limited remaining tiles
- deterministic two-ply score comparison (both player perspectives)
...
2. Add lightweight rollout bias for useful remaining tiles and open movement
  axes to improve early-game quality.
